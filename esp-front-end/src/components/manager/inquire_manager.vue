        ...
        <div class="row">
            <div class="col-md-12">
                <v-client-table ref="myTable" :data="tableData" :columns="columns" :options="options">
                    ...
                    <a slot="reviewed" slot-scope="props" @click="edit(props.row.id)">
                        {{ modifyItem(props.row.reviewed) }}
                    </a>
                </v-client-table>
            </div>
        </div>
    ...
    methods: {
        ...
        searchByReviewed(keywordReviewed) {
            if(this.keywordReviewed){
                Event.$emit("vue-tables.filter::filterByReviewed", this.keywordReviewed);
            }
        },
